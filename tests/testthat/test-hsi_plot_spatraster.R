# Test hsi_plot_spatraster ----
# Produces a ggplot object from a single-layer SpatRaster, optionally
...
test_that("hsi_plot_spatraster returns a ggplot object without coords", {
  result <- hsi_plot_spatraster(test_single)
...
test_that("hsi_plot_spatraster returns a ggplot object with coords", {
  result <- hsi_plot_spatraster(test_single, y = test_coords)
...
test_that("hsi_plot_spatraster does not mutate the caller's raster extent", {
  ...
  ext_before <- as.vector(terra::ext(test_single))

  hsi_plot_spatraster(test_single, y = test_coords)

  expect_equal(as.vector(terra::ext(test_single)), ext_before)
})
...
test_that("hsi_plot_spatraster with y sets correct xmin in mm", {
  mm <- terra::minmax(test_coords)
  expected <- mm["min", "col_um"] * 0.001

  result <- hsi_plot_spatraster(test_single, y = test_coords, units = "mm")
...
test_that("hsi_plot_spatraster with y sets correct xmax in mm", {
  mm <- terra::minmax(test_coords)
...
test_that("hsi_plot_spatraster with y sets correct ymin from negated row_um maximum", {
...
test_that("hsi_plot_spatraster with y sets correct ymax from negated row_um minimum", {
  mm <- terra::minmax(test_coords)
...
test_that("hsi_plot_spatraster units = 'um' sets extent in micrometres", {
  mm <- terra::minmax(test_coords)
  expected <- mm["max", "col_um"]

  result <- hsi_plot_spatraster(test_single, y = test_coords, units = "um")
  built <- ggplot2::ggplot_build(result)
...
test_that("hsi_plot_spatraster errors with non-SpatRaster x", {
  expect_error(hsi_plot_spatraster(x = "not a raster"))
})

test_that("hsi_plot_spatraster errors when x has more than one layer", {
  expect_error(
    hsi_plot_spatraster(x = test_multi),
    "layers"
  )
})

test_that("hsi_plot_spatraster errors with invalid units", {
  expect_error(
    hsi_plot_spatraster(test_single, units = "furlongs")
  )
})

test_that("hsi_plot_spatraster errors when y is missing required coord layers", {
  bad_y <- c(test_single, test_single)
  names(bad_y) <- c("wrong_name_a", "wrong_name_b")

  expect_error(hsi_plot_spatraster(test_single, y = bad_y))
})

test_that("hsi_plot_spatraster errors when x and y have different dimensions", {
  # Aggregate doubles pixel size, halving row and col counts
  small_x <- terra::aggregate(test_single, fact = 2)

  expect_error(
    hsi_plot_spatraster(small_x, y = test_coords),
    "dimensions"
  )
})
