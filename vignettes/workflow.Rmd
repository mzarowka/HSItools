...
reflectance_sp <- reflectance |>
  ...
  tidyr::pivot_longer(dplyr::everything(), names_to = "band", values_to = "reflectance", names_transform = readr::parse_number)
...
reflectance_smooth_sp <- reflectance_smooth |>
  ...
  extract_spectral_profile() |>
  # Pivot longer so for plotting
  tidyr::pivot_longer(dplyr::everything(), names_to = "band", values_to = "reflectance_sg", names_transform = readr::parse_number)
...
Optionally, we can process our data further by removing the continuum, which follows the rule of dividing thespectrum by its bounding box. This way spectrum becomes flatter.
...
plot_ratio570630 <- ratio_570630 |>
  plot_raster_proxy(.hsi_index = names(ratio_570630))
