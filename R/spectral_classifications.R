...
if (is.null(filename) == TRUE) {
  filename <- paste0(raster_src, "/KMEANS_", n_clusters, "clusters_", raster_name, ".tif")
} else {
...
if (use_sampling && terra::ncell(raster_clean) > sample_size) {
  cli::cli_alert_info("Sampling {sample_size} pixels for initial k-means clustering")
  
  # Extract a sample of values
  sample_cells <- sample(1:terra::ncell(raster_clean), min(sample_size, terra::ncell(raster_clean)))
  ...
  cli::cli_alert_info("Using all pixels within the extent for k-means clustering")
...
if (nrow(sample_matrix) < n_clusters) {
  rlang::abort(paste0("Not enough complete data points (", nrow(sample_matrix), 
                     ") for ", n_clusters, " clusters. Try a smaller n_clusters value."))
}

# Perform k-means clustering
cli::cli_alert_info("Performing k-means clustering with {n_clusters} clusters")
kmeans_model <- stats::kmeans(sample_matrix, centers = n_clusters, 
...
band_importance <- tibble::tibble(
  ...
  RelativeImportance = apply(kmeans_model$centers, 2, var) / sum(apply(kmeans_model$centers, 2, var)) * 100
) |>
...
if (nrow(sample_matrix) < max_clusters * 10) {
  cli::cli_alert_warning("Sample size may be too small for reliable cluster analysis. Consider reducing max_clusters.")
}
...
cli::cli_alert_info("Evaluating clusters from {min_clusters} to {max_clusters}")

# Within-cluster Sum of Squares (Elbow method)
if ("elbow" %in% methods) {
  cli::cli_alert_info("Computing Within-cluster Sum of Squares (Elbow method)")
  
  for (k in min_clusters:max_clusters) {
    km <- stats::kmeans(sample_matrix, centers = k, nstart = nstart, iter.max = iter.max)
  ...
  wss_ratio <- -diff(results$metrics$wss) / results$metrics$wss[-length(results$metrics$wss)]
  ...
  # Create elbow plot
  if (plot) {
    p_elbow <- ggplot2::ggplot(results$metrics, ggplot2::aes(x = k, y = wss)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::geom_vline(xintercept = optimal_k_elbow, linetype = "dashed", color = "red") +
      ggplot2::geom_vline(xintercept = alternative_k_elbow, linetype = "dotted", color = "blue") +
      ggplot2::labs(
        title = "Elbow Method for Optimal k",
        subtitle = paste("Suggested k:", optimal_k_elbow, "(red), Alternative:", alternative_k_elbow, "(blue)"),
    ...
    if (!is.null(filename)) {
      elbow_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_ELBOW_", raster_name, ".", ext)
...
if ("silhouette" %in% methods) {
  ...
  if (!requireNamespace("cluster", quietly = TRUE)) {
    cli::cli_alert_warning("Package 'cluster' not available. Skipping silhouette analysis.")
  } else {
    for (k in min_clusters:max_clusters) {
    ...
    optimal_k_silhouette <- results$metrics$k[which.max(results$metrics$silhouette)]
    results$optimal$silhouette <- optimal_k_silhouette
    
    # Create silhouette plot
    if (plot) {
      p_silhouette <- ggplot2::ggplot(results$metrics, ggplot2::aes(x = k, y = silhouette)) +
        ggplot2::geom_line() +
        ggplot2::geom_point() +
        ggplot2::geom_vline(xintercept = optimal_k_silhouette, linetype = "dashed", color = "red") +
        ggplot2::labs(
      ...
      if (!is.null(filename)) {
        silhouette_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_SILHOUETTE_", raster_name, ".", ext)
...
if ("gap" %in% methods) {
  ...
  # Check if the cluster package is available
  if (!requireNamespace("cluster", quietly = TRUE)) {
    cli::cli_alert_warning("Package 'cluster' not available. Skipping gap statistic.")
  } else {
    ...
    optimal_k_gap <- cluster::maxSE(gap_stat$Tab[, "gap"], gap_stat$Tab[, "SE.sim"])
    results$optimal$gap <- optimal_k_gap
    
    # Create gap plot
    if (plot) {
      p_gap <- ggplot2::ggplot(results$metrics, ggplot2::aes(x = k, y = gap)) +
        ggplot2::geom_line() +
        ggplot2::geom_point() +
        ggplot2::geom_vline(xintercept = optimal_k_gap, linetype = "dashed", color = "red") +
        ggplot2::labs(
      ...
      if (!is.null(filename)) {
        gap_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_GAP_", raster_name, ".", ext)
...
if ("ch_index" %in% methods) {
  ...
  for (k in min_clusters:max_clusters) {
    km <- stats::kmeans(sample_matrix, centers = k, nstart = nstart, iter.max = iter.max)
    results$metrics$ch_index[results$metrics$k == k] <- calc_ch_index(km, sample_matrix)
  }
  ...
  if (plot) {
    p_ch <- ggplot2::ggplot(results$metrics, ggplot2::aes(x = k, y = ch_index)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::geom_vline(xintercept = optimal_k_ch, linetype = "dashed", color = "red") +
      ggplot2::labs(
    ...
    if (!is.null(filename)) {
      ch_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_CH_", raster_name, ".", ext)
...
if ("db_index" %in% methods) {
  ...
  for (k in min_clusters:max_clusters) {
    km <- stats::kmeans(sample_matrix, centers = k, nstart = nstart, iter.max = iter.max)
    results$metrics$db_index[results$metrics$k == k] <- calc_db_index(km, sample_matrix)
  }
  ...
  if (plot) {
    p_db <- ggplot2::ggplot(results$metrics, ggplot2::aes(x = k, y = db_index)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::geom_vline(xintercept = optimal_k_db, linetype = "dashed", color = "red") +
      ggplot2::labs(
    ...
    if (!is.null(filename)) {
      db_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_DB_", raster_name, ".", ext)
...
if (plot && length(methods) > 1) {
  ...
  if (length(std_columns) > 0) {
    ...
    p_combined <- ggplot2::ggplot(plot_data, ggplot2::aes(x = k, y = Standardized_Value, color = Method)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::labs(
    ...
    if (!is.null(filename)) {
      combined_file <- paste0(raster_src, "/OPTIMAL_CLUSTERS_COMBINED_", raster_name, ".", ext)
      ggplot2::ggsave(combined_file, p_combined, width = 10, height = 6)
    }
  } else {
    cli::cli_alert_warning("Not enough valid methods to create a combined plot")
  }
}
...
if (length(results$optimal) > 0) {
  ...
  consensus_k <- as.numeric(names(sort(table(optimal_values), decreasing = TRUE)[1]))
  results$optimal$consensus <- consensus_k
  
  cli::cli_alert_success("Consensus optimal number of clusters: {consensus_k}")
  for (method in names(results$optimal)[names(results$optimal) != "consensus"]) {
