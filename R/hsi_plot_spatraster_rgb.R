#' Plot a three layer (pseudo)RGB SpatRaster with physical spatial coordinates
#'
#' @family Plotting
#'
...
#' @param stretch Logical. Apply stretch to scale values to 0–255 before
#'   rendering. Default `FALSE`.
#' @param histeq Logical. Use histogram equalization instead of linear
#'   stretch. Only used when `stretch = TRUE`. Default `FALSE`.
#' @param bylayer Logical. Apply stretch per layer independently. When
#'   `FALSE` all three layers are stretched together, preserving relative
#'   brightness between channels. Only used when `stretch = TRUE`.
#'   Default `FALSE`.
...
#'
#' # Pixel-space plot
#' x_spatraster <- hsi_plot_spatraster_rgb(x)
#'
#' # Physical-space plot in mm
#' x_spatraster <- hsi_plot_spatraster_rgb(x, y = x_coords)
#'
...
hsi_plot_spatraster_rgb <- function(
  ...
  # TODO
  # stretch as an argument
  # histeq as an argument
  # bylayer as an argument
  # update tests
  # update roxygen
  # today it works nice with prestretched SpatRaster
  ...
  # Conditional stretch
  if (stretch) {
    x_deep <- terra::stretch(x_deep, type = type)
  }

  # Create ggplot and tidyterra object
  result <- ggplot2::ggplot() +
    tidyterra::geom_spatraster_rgb(data = x_deep) +
