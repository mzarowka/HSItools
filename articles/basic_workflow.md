...
nm) from the Hyperspectral Imaging Laboratory in Gdańsk.
...
captured at the matching integration time. For example Lumo Scanner
always captures a dark reference per session, so both are always
available.
...
You can control whether processing is done in memory or not. With the
default `in_memory = FALSE`,
{[withr](https://cran.r-project.org/package=withr)} manages temporary
files and removes them once the function finishes. You can write the
result to a permanent file via `filename`, or keep it in memory.
